...
document.getElementById("scrollLeft").addEventListener("click", () => {
  scrollGuides(-1); // -1 indicates scroll left
});

document.getElementById("scrollRight").addEventListener("click", () => {
  scrollGuides(1); // 1 indicates scroll right
});
...
function renderGuides(guides) {
  ...
  guides.forEach((g) => {
    ...
    // Check if guide is already booked
    const guideKey = `booked_${g.name || "Unknown"}`;
    const isBooked = localStorage.getItem(guideKey) === "true";
    ...
    card.innerHTML = `
      ...
      ${isBooked
        ...
        : `<button class="btn-book" data-guide="${escapeAttr(g.name || "Unknown")}" data-price="${escapeAttr(g.rate ? "$" + g.rate + "/day" : "N/A")}">
...
function scrollGuides(px) {
  ...
  // Get card width (300px min-width + 25px gap)
  const cardWidth = 325;

  // Calculate current position and number of cards to scroll
...
// Pause auto-scroll when user manually scrolls or hovers
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (container) {
    // Pause on hover
    container.addEventListener("mouseenter", stopCarouselAutoScroll);
    container.addEventListener("mouseleave", startCarouselAutoScroll);
  ...
  // Stop auto-scroll when manual navigation buttons are clicked
  if (leftBtn) {
...
function initBookingModal() {
  ...
  if (!bookingModal || !successPopup || !closeModal || !confirmBooking || !closeSuccess || !bookingDate) {
  ...
  confirmBooking.addEventListener("click", () => {
    ...
    const guideKey = `booked_${selectedGuide.name}`;
    ...
    successMessage.textContent = `Your booking with ${selectedGuide.name} on ${formattedDate} has been confirmed! We'll send you a confirmation email shortly.`;
    ...
    // Re-render guides to show booked status
    if (lastGuides && lastGuides.length > 0) {
...
// Function to attach booking listeners to all book buttons
function attachBookingListeners() {
  const bookButtons = document.querySelectorAll(".btn-book");
  const bookingModal = document.getElementById("bookingModal");
  const selectedGuideName = document.getElementById("selectedGuideName");
  const selectedGuidePrice = document.getElementById("selectedGuidePrice");

  if (!bookingModal || !selectedGuideName || !selectedGuidePrice) {
    return; // Elements not found
  }

  bookButtons.forEach((button) => {
    button.addEventListener("click", () => {
      // Get guide details from data attributes
      selectedGuide.name = button.getAttribute("data-guide");
      selectedGuide.price = button.getAttribute("data-price");

      // Update modal with guide details
      selectedGuideName.textContent = selectedGuide.name;
      selectedGuidePrice.textContent = selectedGuide.price;
...
// Initialize booking modal on page load
document.addEventListener("DOMContentLoaded", () => {
  initBookingModal();
});
